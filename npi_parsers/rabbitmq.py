    ...
    if isinstance(data[svc_name], list):
        for item in data[svc_name]:
            ...
            if messages > thresholds['critical']:
                plugin_output += 'CRITICAL: {0}: {1} messages!\t'.format(name, messages)
                return_code = 2
            elif messages > thresholds['warning']:
                plugin_output += 'WARNING: {0}: {1} messages!\t'.format(name, messages)
                return_code = 1
            elif messages <= thresholds['ok']:
                plugin_output += 'OK: {0}: {1} messages.\t'.format(name, messages)
            else:
                plugin_output += 'UNKNOWN: {0}: {1} messages.\t'.format(name, messages)
                return_code = 3
    else:
